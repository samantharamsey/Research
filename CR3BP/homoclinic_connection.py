...
if __name__ == '__main__':
    ...
    orbitIC, t_cross = xzplane_xfixed(LyapIC)
    P = t_cross*2
    
    num_manifolds = 200
    stableIC,   stTpoints = get_manifold_ICs(mu, orbitIC, P, stepoff, num_manifolds, 
                                positive_dir=True, stable=True)
    unstableIC, unTpoints = get_manifold_ICs(mu, orbitIC, P, stepoff, num_manifolds, 
    ...
    # sol_alpha = solve_ivp(lambda t, y: EOM(t, y, mu), (t0, tau_alpha), corr_state, 
    #                 dense_output=True, rtol=tol, atol=tol)
    ...
    while error > tol:
        ...
        guess_sol = get_manifold_ICs(mu, orbitIC, P, stepoff, 3, 
                                    positive_dir=True, stable=False, 
                                    tau_alpha=tau_alpha,
        ...
        # integrate from tau_alpha to tau_M
        sol_M = solve_ivp(lambda t, y: EOM_STM(t, y, mu), (t0, tau_M), state4, 
                        dense_output=True, rtol=tol, atol=tol)
        ...
        # constraint vector
        const1 = state4[0:6]-state3
        F = np.concatenate((const1, [state5[1]], [state5[3]]), axis=0)
        ...
        dstate_STM2 = EOM_STM(0, np.concatenate([state2, STM_alpha.reshape(1, 36)[0]]), mu)
        # state portion
        dstate2 = dstate_STM2[0:6]
        # STM portion
        dSTM_alpha = dstate_STM2[6::].reshape(6,6)
        ...
        dstate5 = EOM(0, state5, mu)
        
        # combine to form jacobian
        DF_top = np.concatenate((    I6, drdtau.reshape(6, 1)), axis=1)
        DF_top = np.concatenate((DF_top,     Z6.reshape(6, 1)), axis=1)
        
        DF_r5 = np.concatenate((phi_M2, [0], [dstate5[1]]))
        DF_r6 = np.concatenate((phi_M4, [0], [dstate5[3]]))
    ...
    plot_orbit_2D(ax4, orbitIC, P, 'black')
    plot_orbit_2D(ax4,  state3, tau_M*2, 'magenta')
    
    
    moon = plt.Circle((1-mu, 0), rmoon, color='gray', zorder=3, alpha=0.8)
    ax4.add_patch(moon)
    ax4.set_title('Earth-Moon L1 Lyapunov Homoclinic Connection')
    ...
    #     # get manifold states at current tau-alpha guess
    #     unstableIC1, unTpoints1, stvecs, unvecs, STMs, fixedpoints = get_manifold_ICs(mu, orbitIC, P, stepoff, 3, 
    #                                 positive_dir=True, stable=False, tau_alpha=tau_alpha,
    #                                 return_eigvecs=True, return_STMs=True, return_fixedpoints=True)
    ...
    #     # constraint vector
    #     F = np.array([(state4[0:6]-state3), state5[1], state5[3]], dtype=object)
    #     print(F)
    #     F = np.array([(state4[0:6]-state3)*I6, state5[1]*I6, state5[3]*I6])
    ...
    #     # combine to form jacobian
    #     DF = np.array([[   I6, drdtau,         Z6],
    #                   [phi_M2,     Z6, dstate5[1]],
    #                   [phi_M4,     Z6, dstate5[3]]], dtype=object)
        
    #     DF = np.array([[      I6, drdtau*I6,            Z6],
    #                   [phi_M2*I6,        Z6, dstate5[1]*I6],
    #                   [phi_M4*I6,        Z6, dstate5[3]*I6]])
        
        
    #     sol = np.linalg.pinv(DF)@F
        
    #     state4_update    = np.diagonal(sol[0][1])
    #     tau_alpha_update = np.diagonal(sol[1][2])[0]
    #     tau_M_update     = np.diagonal(sol[2][2])[0]
        
        
    #     # update the free variable vector
    #     X = np.array([state4[0:6]-state4_update, tau_alpha-tau_alpha_update, tau_M-tau_M_update], dtype=object)
        
    #     return X
    
    # X1 = homoclinic_update(X, corr_state, t_cross*2)
    # X2 = homoclinic_update(X1, corr_state, t_cross*2)
    ...
    #     # integrate from t0 to tau_alpha
    #     sol_alpha = solve_ivp(lambda t, y: EOM(t, y, mu), (t0, X[1]), corr_state, 
    #                     dense_output=True, rtol=tol, atol=tol)
    #     ax.plot(sol_alpha.y[0], sol_alpha.y[1])
        
    #     # integrate from tau_alpha to tau_M
    #     sol_M = solve_ivp(lambda t, y: EOM(t, y, mu), (t0, X[2]), X[0], 
    #                     dense_output=True, rtol=tol, atol=tol)
    #     ax.plot(sol_M.y[0], sol_M.y[1])
        
    #     # ax3.legend(['Segment 1', 'Segment 2'])
    # plot_guesses(ax3, X, corr_state)
    # plot_guesses(ax3, X1, corr_state)
    # # plot_guesses(ax3, X2, corr_state)
